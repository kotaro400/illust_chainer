...
const Pictures = () => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL}/pictures`).then((res) => {
  ...
  if (pictures.length === 0) {
    ...
    return (
      <PicturesComponent>
        <MainHeader />
        <LastPicture
          order={pictures[0].order}
          image_url={pictures[0].image_url}
        />
        <Button type="primary" shape="round" size="large">
          <Link to={`/new/${pictures[0].chain_id}/${pictures[0].order + 1}`}>
            次の絵を描く
          </Link>
        </Button>
